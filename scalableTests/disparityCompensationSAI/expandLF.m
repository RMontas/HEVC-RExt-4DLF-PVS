...
fileID_L34_REC_SAIs = fopen('LF_Buff_L34_REC_SAIs.yuv', 'w' );
fileID_L56_REC_SAIs = fopen('LF_Buff_L56_REC_SAIs.yuv', 'w' );
frameNum = 1;
for l = 1:num_Layers
    for j = 1:miSize
        for i = 1:miSize 
            ...
            if layerMask(ypos,xpos) == l
                ...
                if l <= 6
                    LF_L56(ypos,xpos,:,:,1) = uint16(Y(1:H_PVS_wo_padding,1:W_PVS_wo_padding));
                    LF_L56(ypos,xpos,:,:,2) = uint16(U(1:H_PVS_wo_padding,1:W_PVS_wo_padding));
                    LF_L56(ypos,xpos,:,:,3) = uint16(V(1:H_PVS_wo_padding,1:W_PVS_wo_padding));
                end
                            ...
                            for iSAI = 10:45
                                ...
                                if expandToLF7((yposNonCausal+1)/2,(xposNonCausal+1)/2) > 0 && expandToLF7((yposNonCausal+1)/2,(xposNonCausal+1)/2) < 8
                                    if expandToLF7((yposNonCausal+1)/2,(xposNonCausal+1)/2) == 1 % horizontal
                                        ...
                                        if winSym == 1
                                            dispCompensationSearchWindow = [-winDispComp(2) -winDispComp(1); winDispComp(2) winDispComp(1)];
                                        end
                                    end
                                    if expandToLF7((yposNonCausal+1)/2,(xposNonCausal+1)/2) == 2 % vertical
                                        ...
                                        if winSym == 1
                                            dispCompensationSearchWindow = [-winDispComp(1) -winDispComp(2); winDispComp(1) winDispComp(2)];
                                        end
                                    end
                                    if expandToLF7((yposNonCausal+1)/2,(xposNonCausal+1)/2) == 3 % diagonal L \
                                        ...
                                        if winSym == 1
                                            dispCompensationSearchWindow = [-winDispComp(1) -winDispComp(1); winDispComp(1) winDispComp(1)];
                                        end
                                    end
                                    ...
                                    [ dx dy ] = disparityCompensationSAI_BM( squeeze(PVS(referenceSAI0,:,:,1)), squeeze(PVS(referenceSAI1,:,:,1)), dispCompensationSearchWindow, blockSize);
                                    compensatedSAIY = compensateDisparitySAI( squeeze(PVS(referenceSAI0,:,:,1)), round(dx/2), round(dy/2) );
                                    compensatedSAIU = compensateDisparitySAI( squeeze(PVS(referenceSAI0,:,:,2)), round(dx/2), round(dy/2) );
                                    compensatedSAIV = compensateDisparitySAI( squeeze(PVS(referenceSAI0,:,:,3)), round(dx/2), round(dy/2) );
                                    fwrite(fileID_L34_REC_SAIs, permute(compensatedSAIY, [2 1 3]), 'uint16');
                                    fwrite(fileID_L34_REC_SAIs, permute(compensatedSAIU, [2 1 3]), 'uint16');
                                    fwrite(fileID_L34_REC_SAIs, permute(compensatedSAIV, [2 1 3]), 'uint16');
                                    for y = 1:H_PVS_wo_padding
                                        for x = 1:W_PVS_wo_padding
                        ...
                        if frameNum == 46
                            for iSAI = 46:169
                                ...
                                if expandToLF13(yposNonCausal,xposNonCausal) > 0 && expandToLF13(yposNonCausal,xposNonCausal) < 8
                                    ...
                                    compensatedSAIU = compensateDisparitySAI( squeeze(PVS(referenceSAI0,:,:,2)), round(dx/2), round(dy/2) );
                                    compensatedSAIV = compensateDisparitySAI( squeeze(PVS(referenceSAI0,:,:,3)), round(dx/2), round(dy/2) );
                                    fwrite(fileID_L56_REC_SAIs, permute(compensatedSAIY, [2 1 3]), 'uint16');
                                    fwrite(fileID_L56_REC_SAIs, permute(compensatedSAIU, [2 1 3]), 'uint16');
                                    fwrite(fileID_L56_REC_SAIs, permute(compensatedSAIV, [2 1 3]), 'uint16');
                                    for y = 1:H_PVS_wo_padding
                                        for x = 1:W_PVS_wo_padding
